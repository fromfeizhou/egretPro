/**
 * Created by Tint_ on 2017/3/15.
 */
import EaseType = egret.tween.EaseType;
 ...
 */
class UIElement extends UIBaseElement {

    protected static _mInstance: any = null;
    private _mShowParent: egret.DisplayObjectContainer = null;
    /**是否遮罩*/
    private _mIsMark: boolean = false;
    private _mUiMask: egret.Sprite = null;
    /**遮罩透明度*/
    private _mMaskAlpha: number = 0.3;

    public _mIsShow: boolean = false;//是否show了
    public _mInit: boolean = false;
    private _mLoadingGro: eui.Group = null;
    private _mUIShowType: UIShowStyle;
    private _mUIAlignType: UIAlignStyle;
    protected touchClose = false;
     ...
     */
    public constructor(parent: egret.DisplayObjectContainer, skinName: string = "",
        alignStyle: UIAlignStyle = UIAlignStyle.Normal,
        ...
        this.name = "Exml_" + skinName;
        this.osReSize( skinName );
        let self = this;
    ...
    public initMask(isTouch: boolean) {
        this._mIsMark = true;
    ...
    private _tempN: number = 0;
    private showIt(): void {
        let self = this;
        if (self._mIsShow == false || !self._mInit) return;
    ...
    private _mRecordX: number = 0;
    private _mRecordY: number = 0;
    private _mRecordScaleX: number = 1;
    private _mRecordScaleY: number = 1;
    private _mAnchorX: number = 0;
    private _mAnchorY: number = 0;
    private static elasticOut = Ease.getElasticOut(1, 1);
    private showRealize() {
        let self = this;
        switch (self._mUIShowType) {
            case UIShowStyle.ScaleStyle:
                ...
                let sx: number = (self._mRecordScaleX / 10);
                let sy: number = (self._mRecordScaleY / 10);
    ...
    private removeChildrens(container: egret.DisplayObjectContainer) {
        let tc: any = null;
    ...
    protected touchUiMask(){
        this.slideHide();
    }
    protected restPosition() { };
    ...
    public tweenHide( callback,tagret ){
        let self = this;
        self._mRecordX = self.x;
        self._mRecordY = self.y;
        egret.Tween.get(self)
