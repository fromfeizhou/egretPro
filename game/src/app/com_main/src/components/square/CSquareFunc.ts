module com_main {
	export class CSquareFunc {
		/**
		 * 根据状态获取动画对应的状态字符
		 */
		public static getActionCharByStatus(status: CSquare_Status): string {
			var strStatus = "";
			switch (status) {
		 ...
		 * 根据状态获取士兵位置
		 */
		public static getPositionListConfig(status: CSquare_Status, id: number, direction: CSquare_Direction, targetNumber?: number, oldDirection?: CSquare_Direction): string {
			let config;
			let configId = "";
			var strStatus = "";
			let code = 0;
		 ...
		 * 根据状态获取士兵位置
		 */
		public static getAttackEffectId(direction: CSquare_Direction, isplayer: boolean) {
			let effectId = 0;
			let isReversal = false; //是否翻转
			switch (direction) {
		 ...
		 * 根据方向获取动画对应的方向字符
		 */
		public static getActionCharByDirection(direction: CSquare_Direction, oldDirection?: CSquare_Direction): CSquare_Direction {
			switch (direction) {
				case CSquare_Direction.LEFT_UP:
					direction = CSquare_Direction.RIGHT_UP;
					break;
		 ...
		 * 根据方向方阵朝向  斜上 斜下 4个方向
		 */
		public static getFourDirection(direction: CSquare_Direction, oldDirection?: CSquare_Direction): CSquare_Direction {
			switch (direction) {
				case CSquare_Direction.LEFT_UP:
					direction = CSquare_Direction.LEFT_UP;
					break;
		 ...
		 * 根据方向获取特效旋转角度
		 */
		public static getEffectRotationByDirection(direction: CSquare_Direction): number {
		 ...
		 * 根据旋转角度获取特效方向
		 */
		public static getEffectDirectionByRotation(angle) {
			let temp: CSquare_Direction = CSquare_Direction.RIGHT;
		 ...
		 * 获取方阵排列
		 */
		public static getSquareGrid(type: CSquare_Type): any {
			var grid = null;
			var row = 3;
			var num = 0;
			switch (type) {
		 ...
		 * 获取对象的中心点
		 */
		public static getObjectCenter(target: egret.DisplayObject): any {
			var point = {};
		 ...
		 * 画对象区域
		 */
		public static drawTargetArea(target: egret.DisplayObject, color: number = 0x00FF00) {
		 ...
		 * dis 炸飞距离
		 */
		public static flick(obj: egret.DisplayObject, gzX: number = 0, gzY: number = 0, dis: number = 10) {
			var ox = obj.x;
			var oy = obj.y;
			var dx = obj.x - gzX;
			var dy = obj.y - gzY;
		 ...
		 * 飘血效果
		 */
		public static hpChangeAction(obj: egret.DisplayObject, height: number, callback: Function, thisArg: any) {
			obj.y = - 80;
			obj.alpha = 0;
			egret.Tween.get(obj)
				.to({ y: -35, alpha: 1 }, 100)
		 ...
		 * 
		 */
		public static battleInfoAction(obj: egret.DisplayObject, callback: Function, thisArg: any) {
			obj.scaleX = 0.8;
			obj.scaleY = 0.8;
			obj.alpha = 0;
			egret.Tween.get(obj)
		 ...
		 * 突击动画
		 */
		public static strikeAction(obj: egret.DisplayObject) {
		 ...
		 * 获取死亡后退位置
		 */
		public static getDiePosition(direction: CSquare_Direction, oldDirection: CSquare_Direction) {
			//斜度按30度算
			let step = ConstUtil.getValue(IConstEnum.BATTLE_SOLDIER_DIE_DIS);
