...
class MapUtil {
	/**初始化地图数据 */
	public static initMapData(mapid: number) {
		let setting = MapLoader.loader(mapid);
	...
	/**格仔转像素 */
	public static cellToPixel(cell: Cell): egret.Point {
		let pos: number[] = ISOMap.getInstance().cellToPixel(cell.getCellX(), cell.getCellY());
		if (!pos) return null;
		return new egret.Point(pos[0], pos[1]);
	}

	/**像素转格仔 */
	public static pixelToCell(x: number, y: number): Cell {
		let pos: number[] = ISOMap.getInstance().pixelToCell(x, y);
		if (!pos) return null;
